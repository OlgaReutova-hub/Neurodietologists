...
memo_pdf_path = project_root / "Памятка для пациента.pdf"
...
def show_pdf_memo_dialog() -> None:
    if memo_pdf_path is None or not memo_pdf_path.exists():
        st.error("Файл памятки PDF не найден.")
        return
    ...
    # Use Blob URL instead of data: URL to reduce browser-side blocking.
    components.html(
        f"""
        <div style="border:1px solid #e5e7eb; border-radius:12px; overflow:hidden;">
            <iframe id="memo-frame" width="100%" height="820" style="border:0;"></iframe>
        </div>
        <script>
            ...
            document.getElementById("memo-frame").src = blobUrl;
        </script>
        """,
        height=840,
    )
